...
const findFile = ({ id, hash, name, url, alternativeText, caption }, user, allowedFileTypes) => __awaiter(void 0, void 0, void 0, function* () {
    let file = null;
    if (!file && id) {
        file = yield strapi.entityService.findOne('plugin::upload.file', id);
    }
    if (!file && hash) {
        [file] = yield strapi.entityService.findMany('plugin::upload.file', { filters: { hash }, limit: 1 });
    }
    if (!file && name) {
    ...
    if (!file && url) {
        ...
        if (checkResult.isValid) {
            ...
            if (!file) {
                file = yield importFile({ id: id, url: checkResult.fileData.rawUrl, name: name, alternativeText: alternativeText, caption: caption }, user);
            }
        }
    }
    return file;
});
const importFile = ({ id, url, name, alternativeText, caption }, user) => __awaiter(void 0, void 0, void 0, function* () {
    let file;
    try {
        ...
        if (id) {
            uploadedFile = yield strapi.db.query('plugin::upload.file').update({
...
const getFileDataFromRawUrl = (rawUrl) => {
    ...
    const hash = ((_b = (0, last_1.default)(parsedUrl.pathname.split('/'))) === null || _b === void 0 ? void 0 : _b.slice(0, -(extension.length + 1))) || '';
    return {
        hash,
        name,
        extension,
